...
export default async function importPlaypuff({ container }: ExecArgs) {
  ...
  if (!file) throw new MedusaError(MedusaError.Types.INVALID_DATA, "Set PLAYPUFF_CATALOG_FILE to the supplier catalog JSON. Default is dry-run; set PLAYPUFF_IMPORT_APPLY=true to write validated rows.")
