...
type Result = { dry_run: boolean; products: { sku: string; operation?: string; status?: string; product_id?: string }[] }

type Input = { rows: unknown; apply: boolean }

const importToysStep = createStep("import-toys", async (input: Input, { container }): Promise<StepResponse<Result>> => {
  ...
  for (const row of rows) {
    ...
    if (!supplier.is_active) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: supplier is inactive`)
    if (!channel.stock_locations?.some(l => l?.id === row.location_id)) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: stock location is not linked to Toys`)
    const { data: categories } = await query.graph({ entity: "product_category", fields: ["id"], filters: { id: row.category_ids } })
    if (categories.length !== row.category_ids.length) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: missing category`)
    const { data: profiles } = await query.graph({ entity: "shipping_profile", fields: ["id"], filters: { id: row.shipping_profile_id } })
    if (!profiles.length) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: missing shipping profile`)
    ...
    if (variant && (!variant.product?.sales_channels?.some(c => c?.id === channel.id) || variant.product.variants?.length !== 1 || variant.product.handle !== row.handle)) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: existing product must be a single-SKU Toys product with the same handle`)
    if (!variant) {
      const { data: sameHandle } = await query.graph({ entity: "product", fields: ["id"], filters: { handle: row.handle } })
      if (sameHandle.length) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: handle already belongs to another product`)
    }
  ...
  for (const item of prepared) {
    ...
    if (!inventory) throw new MedusaError(MedusaError.Types.INVALID_DATA, `${row.sku}: inventory missing; product remains draft`)
...
export const importToysWorkflow = createWorkflow("import-toys", (input: Input) => new WorkflowResponse(importToysStep(input)))
