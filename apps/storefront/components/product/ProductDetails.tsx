...
function wornGalleryImagesFor(handle: string): { id: string; url: string }[] {
  return ["worn-victor", "worn-victor-male"].flatMap((dir) =>
    VICTOR_POSES.map((pose) => ({
      id: `${dir}-${pose}`,
      // A new filename refreshes cached portraits without breaking Next Image's
      // optimizer, which rejects query parameters on local image paths.
      url: `/tryon/${dir}/${handle}-${pose}${
    ...
    <div className="container-page max-w-6xl grid gap-8 py-6 pb-20 md:grid-cols-[auto_1fr] md:gap-12 lg:gap-16 md:py-8 md:pb-12 items-start">
      ...
      <div className="min-w-0 md:sticky md:top-24 md:self-start">
        <div className="flex gap-4 items-start">
          ...
          <div className="relative aspect-[4/5] w-[320px] sm:w-[380px] md:w-[400px] max-h-[500px] overflow-hidden bg-card flex items-center justify-center rounded-sm">
          ...
          <div className="mt-4 flex gap-3 overflow-x-auto sm:hidden">
            {images.map((im, i) => (
              <button
                ...
                onClick={() => setActiveImg(i)}
                aria-label={`Image ${i + 1}`}
                className="relative h-14 w-14 shrink-0 overflow-hidden border border-token bg-card rounded-sm"
                style={i === activeImg ? { borderColor: "var(--color-fg)", opacity: 1 } : { opacity: 0.55 }}
              >
                {im.url && (
                  <Image
